    ...
    void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
    {
        ...
        #if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
            rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float4(1,0,0,1);
        #else
        ...
        rimWidth = max(lerp(rimWidth, rimWidth * lerp(0, 1, poiLight.lightMap - float(0.5)) * float(0.5), float(0.2)), 0);
        rim = 1 - smoothstep(min(float(0.1), rimWidth), rimWidth, viewDotNormal);
        rim *= float4(1,0,0,1).a * rimColor.a * rimMask;
        rimLightEmission = rim * lerp(albedo, rimColor, float(0)) * float(0.1);
