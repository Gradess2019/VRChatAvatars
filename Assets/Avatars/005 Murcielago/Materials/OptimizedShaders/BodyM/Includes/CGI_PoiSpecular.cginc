    ...
    float3 calculateSpecular(in float4 albedo)
    {
        ...
        if(float(1) == 1) // Realistic
        {
            ...
            finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(0.2094424,0,0,1), metallic, float(0.9) * specularMap.a, poiLight.dotNH, poiLight.dotLH, poiLight.color, attenuation);
        }
        ...
        if(float(1) == 4)
        {
            ...
            finalSpecular += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap, poiLight.nDotL, float(0.95), float(1), float4(0.2094424,0,0,1), albedo, metallic, jitter, float(0), packedTangentMap);
            ...
            if(float(1) == 2) // Toon
            {
                ...
                finalSpecular *= float4(0.2094424,0,0,1);
            }
            ...
            if (float(1) == 3) // anisotropic
            {
                ...
                finalSpecular += AnisotropicSpecular(float(0), float(0), float(0.9), float(0), float(1), float(1), float4(0.2094424,0,0,1), metallic, specularMap, poiLight.color, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
                finalSpecular *= float4(0.2094424,0,0,1);
                finalSpecular *= attenuation;
            }
        #endif
            ...
            for (int index = 0; index < 4; index ++)
            {
                ...
                if (float(1) == 1) // Realistic
                {
                    finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(0.2094424,0,0,1), metallic, float(0.9) * specularMap.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
                }
            }
        #endif
        
        finalSpecular *= float4(0.2094424,0,0,1).a;
