    ...
    void applyMatcap(inout float4 finalColor, inout float3 matcapEmission)
    {
        ...
        matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(0.5775805,0.5775805,0.5775805,1);
        matcap.rgb *= float(1);
        ...
        blendMatcap(finalColor, float(0), float(0), float(0.02), matcap, matcapMask, matcapEmission, float(0.015)
        #ifdef POI_LIGHTING
            , float(0)
        #endif
        #ifdef POI_BLACKLIGHT
