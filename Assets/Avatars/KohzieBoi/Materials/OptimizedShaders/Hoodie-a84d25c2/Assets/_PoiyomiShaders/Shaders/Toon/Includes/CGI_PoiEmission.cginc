    ...
    float3 calculateEmissionNew(in float4 baseColor, inout float4 finalColor)
    {
        ...
        float3 emissionColor1 = 0;
            ...
            if(float(1))
            {
                ...
                if(float(0))
                {
                    emissionStrength1 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve1, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(0)], _EmissionScrollingCurve1) + (dot(pos1, float4(100,0,0,0)) * float(40)) + _Time.x * float(8));
                }
                else
                {
                    emissionStrength1 *= calculateScrollingEmission(float4(100,0,0,0), float(8), float(40), float(8), float(0), pos1);
                }
            }
