    ...
    float3 calculateEmissionNew(in float4 baseColor, inout float4 finalColor)
    {
        ...
        if (!float(0))
        {
            emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * lerp(1, baseColor, float(0)).rgb * float4(1,1,1,0).rgb;
        }
        else
        {
            emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)) * lerp(1, baseColor, float(0)).rgb * float4(1,1,1,0).rgb;
        }
        ...
        if(float(0))
        {
            ...
            if(float(0))
            {
                emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, TRANSFORM_TEX(poiMesh.uv[float(0)], _EmissionScrollingCurve) + (dot(pos, float4(0,-10,0,0)) * float(7)) + _Time.x * float(1.34));
            }
            else
            {
                emissionStrength0 *= calculateScrollingEmission(float4(0,-10,0,0), float(1.34), float(7), float(10), float(0), pos);
            }
        }
